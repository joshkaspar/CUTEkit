...
def build_setup_sh_text(apt_packages, curl_tools, custom_setup_tools, system_config, shell_setup_file=".zshrc.setup"):
    ...
    lines.append('mkdir -p "$USER_HOME/CUTEkit-main"')
    lines.append('cp "$SCRIPT_DIR/99-lock-doors.sh" "$USER_HOME/CUTEkit-main/99-lock-doors.sh"')
    lines.append('cp "$SCRIPT_DIR/4-post-install-steps.md" "$USER_HOME/CUTEkit-main/4-post-install-steps.md"')
    lines.append('chown -R "$TARGET_USER:$TARGET_USER" "$USER_HOME/CUTEkit-main"')
    ...
    # The user must confirm key-based login works from a new terminal before
    # running 99-lock-doors.sh — and must run that script from the new session,
    # not this root session. THIS_IP makes the SSH command copy-paste ready.
    lines.append('THIS_IP=$(hostname -I | awk \'{ print $1 }\')')
    lines.append('log "Setup complete!"')
    lines.append('echo ""')
    lines.append('printf "\\033[1;33m"')
    lines.append('echo "  Before running the hardening script:"')
    lines.append('echo ""')
    lines.append('echo "    Confirm you can open a NEW terminal and log in with your key:"')
    lines.append('echo "      ssh $TARGET_USER@$THIS_IP"')
    lines.append('echo ""')
    lines.append('echo "    If that works, run the hardening script from that new session:"')
    lines.append('echo "      bash ~/CUTEkit-main/99-lock-doors.sh"')
    lines.append('echo ""')
    lines.append('echo "    The hardening script disables password auth. There is no undo."')
    lines.append('printf "\\033[0m"')
    lines.append('echo ""')
    lines.append('printf "\\033[1;36m"')
    lines.append('echo "  Next: review your post-install checklist:"')
    lines.append('echo "    cat ~/CUTEkit-main/4-post-install-steps.md"')
    lines.append('printf "\\033[0m"')
    lines.append('echo ""')
    lines.append("")
...
def build_lock_doors_sh_text(system_config):
    ...
    lines.append("# Run this only after confirming key-based SSH login works in a new terminal.")
    ...
    # Ask the user to explicitly confirm before proceeding. This is a deliberate
    # speed bump — it forces anyone who runs this script to have read the prompt
    # and consciously confirm that key-based login is already working.
    lines.append('printf "\\033[1;33m"')
    lines.append('echo "  This script disables password authentication and root login."')
    lines.append('echo "  There is no undo."')
    lines.append('echo ""')
    lines.append('echo "  Only proceed if you have already confirmed that key-based SSH"')
    lines.append('echo "  login works for your user in a separate terminal session."')
    lines.append('printf "\\033[0m"')
    lines.append('echo ""')
    lines.append('read -rp "Have you confirmed key-based login works? [y/N] " CONFIRMED')
    lines.append('if [[ "${CONFIRMED,,}" != "y" ]]; then')
    lines.append('    echo "Aborted. Run this script again once you have confirmed key login."')
    lines.append("    exit 1")
    lines.append("fi")
    lines.append("")
